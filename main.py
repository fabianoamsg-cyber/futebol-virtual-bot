...
LAMBDA_TABLE = [(1.70,1.78,3.0,1.73),(1.79,1.92,2.8,1.88),(1.93,2.10,2.6,2.07),(2.11,2.35,2.4,2.33),(2.36,2.70,2.2,2.65)]
...
def extract_by_locators(page):
    o15=o25=btts=None
    # tenta achar células contendo '1.5' e '2.5' e pegar a odd da mesma linha (primeira numérica à direita)
    rows = page.locator("text=Gols Mais/Menos").first.locator("xpath=..").locator("xpath=following::*").all()
    try:
        html = page.content()
        # fallback: varrer por números depois do label
    ...
    try:
        ...
        if node:
            ...
            sim = parent.locator("text=Sim").first
            if sim:
                # pega o próximo número no DOM
                html = parent.inner_html()
                m = re.search(r"Sim[\s\S]{0,40}?([1-9]\d*(?:[.,]\d{1,2})?)", html)
...
def with_browser(fn):
    def _wrap():
        with sync_playwright() as p:
            ...
            page.wait_for_timeout(8000)  # dar tempo do grid aparecer
            res = fn(page)
...
def scan_once(page):
    ...
    print("ODDS EXTRAIDAS:", odds)  # aparece nos logs do Render
    signal, err = decide_and_text(odds)
    if err or not signal: return {"odds":odds,"sent":False,"reason":err}
    _, text = signal
    ...
    return {"odds":odds,"sent":False,"reason":"duplicated"}
